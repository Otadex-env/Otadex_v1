...
- Dernière mise à jour : **Task 58 — Fix double-encodage URL + scripts automation animé**, 09 juin 2026
- Task 58 : Fix double-encodage URL images (galerie 404 → 200). Diagnostic : `Uri.encodeFull()` encode `%` → `%25`, donc appeler `encodeFull` sur une URL déjà précodée (`%20`) produisait `%2520` (404 GitHub). Fix dans `otadex_image.dart` et `image_prefetcher.dart` : `_normalizeUrl()` fait `Uri.encodeFull(Uri.decodeFull(url))` — idempotent sur URLs brutes ET pré-encodées. Création de `scripts/setup_anime.js` : parse .docx (mammoth), génère URLs, met à jour `app_assets.dart` / `app_colors.dart` / `firestore_character_service.dart`, génère `import_[prefix].js`, lance import Firestore, crée `temp_images/[Animé]/[Personnage]/`. Création de `scripts/push_images.js` : scanne `temp_images`, renomme convention `[prefix]_[slug]N.jpeg` + conversion JPEG (sharp), copie vers `otadex-assets`, git push, met à jour `app_assets.dart` + Firestore `images[]`. `package.json` : `sharp` ajouté. `ANIME_WORKFLOW.md` entièrement réécrit avec la procédure 2-commandes.
