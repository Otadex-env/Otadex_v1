...
- Dernière mise à jour : **Task 62 — Mode développeur Kage bypass + menu dev ProfileScreen**, 12 juin 2026
- Task 62 : Override rang développeur sans Firestore. `user_profile_provider.dart` : constante `kDeveloperUids = ['tYTfcUyV76MTQCEWuwq1yxTuuAH3']` (UID lu depuis `.env`) + `devOverrideRankProvider` (StateProvider.autoDispose<UserRank?>). `main.dart` : dans `addPostFrameCallback`, si UID ∈ `kDeveloperUids` → force `rank = kage` en mémoire sans écriture Firestore ; dans `_checkLicenseExpiry`, early return si UID développeur (jamais rétrograder vers Genin). `profile_screen.dart` : compteur `_devTapCount` — 7 taps sur la zone héro ouvrent le BottomSheet "🛠 Mode Développeur" (kDebugMode + UID développeur requis) ; 3 boutons Genin/Jonin/Kage → `updateIdentity(rank)` + `OtadexThemeWrapper.updateRank()` + `devOverrideRankProvider` mis à jour + SnackBar ; bouton "Réinitialiser (Kage réel)" → restaure kage. Aucune modification Firestore dans tout ce flow. `dart analyze lib/` : 0 erreur.
- Task 61 : Vérification et consolidation des scripts d'automatisation animés. **Point 1** — logique `toVarName` identique dans les deux scripts (pas de divergence) → extraite dans `scripts/anime_workflow/naming.js` (source unique) ; `setup_anime.js` et `push_images.js` importent désormais `toVarName` depuis ce module. **Point 2** — variable morte `urlList` supprimée de `updateAppAssets()` dans `push_images.js` (calculée, jamais utilisée). **Point 3** — avertissement explicite ajouté : si une variable est absente de `app_assets.dart` (ni `[]` vide ni remplie), affiche `⚠️ [charName] : variable "${varName}" introuvable — vérifier setup_anime.js`. **Point 4** — flag `--dry-run` ajouté sur `push_images.js` : effectue renommage/conversion (étape B), affiche copies/URLs/état variables `app_assets.dart`, sort sans copier vers `otadex-assets`, sans git push, sans relancer l'import Firestore. `ANIME_WORKFLOW.md` : section "Automatisation (scripts)" ajoutée (setup_anime, push_images, --dry-run, naming.js).
- Task 60 : Import Mushoku Tensei exécuté avec succès dans Firestore. Correction `google_time_offset.js` : `OFFSET_MS = -3600000` (horloge locale en avance de 60 min sur Google). Résultat import : `creators/rifujin-na-magonote`, `studios/studio-bind`, `animes/mushoku-tensei`, 20 docs `characters/mt-*`, 7 docs `quizzes/mt-*`. Refonte home — `character_grid_section.dart` : 3 sections distinctes — **🕐 Récents** (triés `created_at` desc), **⭐ Recommandés pour toi** (filtré par genres onboarding), **🔥 Tendances** (triés `likesCount` global). `character.dart` : champ `createdAt` (DateTime?) ajouté. `firestore_character_service.dart` : mapping `Timestamp` → `DateTime` sur `created_at`. `otadex_providers.dart` : `recentCharactersProvider` ajouté (FutureProvider.family). `dart analyze` : 0 erreurs.
